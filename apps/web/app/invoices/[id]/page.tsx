...
export default function InvoiceDetailPage() {
    ...
    function dlXml() {
        if (!inv?.xmlPath) return;
        download(`/invoices/${inv.id}/download-xml`, `${inv.number}.xml`).catch(() => toast.error('Download failed'));
    }

    function dlPdf() {
        if (!inv?.pdfPath) return;
        download(`/invoices/${inv.id}/download-pdf`, `${inv.number}.pdf`).catch(() => toast.error('Download failed'));
    }
            ...
            <Card>
                <CardContent className="p-6 space-y-4">
                    <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <Button variant="outline" disabled={!inv.xmlPath} onClick={dlXml}>Download XML</Button>
                        <Button variant="outline" disabled={!inv.pdfPath} onClick={dlPdf}>Download PDF</Button>
                    </div>
